...
**Status:** Architecture decision | **Date:** 2026-05-18
...
We use Babel for **cross-site routing only**. Local-mode (Mode 1, single L2 bridge) needs no routing — every device is on the same broadcast domain and the kernel handles delivery.
...
See [dhcp-crdt.md §2](dhcp-crdt.md#2-data-model) for the merged data model.

---

## 4. Tunnel Interface Model

Each Iroh QUIC connection to a remote peer surfaces as a numbered point-to-point TUN interface managed by the daemon:
...
> **Open implementation question:** single shared `mj-tun0` with daemon-side per-peer dispatch vs. one TUN per peer. The single-TUN model is leaner on FD count but forces the daemon to multiplex Babel packets per destination, which is fiddly. The per-peer-TUN model lets babeld handle interface semantics natively. Recommending per-peer TUN unless TUN exhaustion becomes a real constraint at scale.

---

## 5. babeld Configuration

Per-router config (`/etc/babeld.conf`), generated by the mjolnir-mesh daemon at startup and on tunnel events:

```
# Daemon manages: interface lines per-peer, redistribute filter for local subnets

interface mj-peer-aabbccdd type tunnel
interface mj-peer-eeff0011 type tunnel

redistribute ip 10.42.1.0/24 ge 24 le 24 allow   # this router's claimed subnet
redistribute local deny                          # nothing else
redistribute proto static deny
redistribute proto kernel deny

# do not advertise the link /31s themselves
in ip 10.255.0.0/16 deny
out ip 10.255.0.0/16 deny

# Babel-specific: long hold for tunnels, short for direct links
default rxcost 96
```
...
Babel was originally an IPv6-leaning protocol but RFC 8966 codifies dual-stack and IPv4-only operation. Single-stack IPv4 is fully supported and is what we ship. See [the IPv4-only design discussion](mesh-network-coordination.md) for rationale.
...
- **babeld** — OpenWrt package (`opkg install babeld`). ~250 KB installed. No Rust binding required; the daemon writes config and signals babeld via SIGHUP / process management.
...
1. **TUN-per-peer vs shared TUN** (§4). Per-peer is the working assumption.
...
- **Related docs**:
  - `dhcp-crdt.md` — CRDT data model with revised `/subnets/` namespace
  - `network-architecture.md` — cross-site packet flow and subnet allocation
  - `mesh-network-coordination.md` — overall architecture
