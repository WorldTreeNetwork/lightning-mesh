...
> **Status 2026-07-02.** The client data plane is CLOSED out and
> field-validated on deployed code — beads **kf7**, **659**, **eon**, **nrr**,
> and **apo** are all closed: **kf7** — the babeld redistribute filter is
> fixed, claimed /24s announce without hand-edits; **659** — the claimed /24
> owns `br-lan` + dnsmasq via UCI (stock `192.168.1.1/24` kept as
> wired-recovery alias); **eon** — claim lifecycle fixed and gossip addresses
> seeded; wr3000s-a now holds its own claim and is no longer parked; **nrr** —
> stray babelds are reaped on service start, fleet-validated; **apo** — client
> transit (internet + cross-LAN) validated end-to-end on deployed code.
> Remaining open work in this runbook's scope: **b9a** (section 1.4
> iperf3/throughput baseline) and **0pv** (section 1.5 physical-client
> matrix).
