...
| Two cards of one origin | One card per service name, a `.mesh` name host is one service name, and IP hosts never embed. A republish that changes the entry origin closes the open card, so an old-record card can't linger beside the new one. Tapping an already-open app only focuses its card, and a pending request continues. A request ends only per the outcome table: visitor close or host removal, app reload, or an origin-changing republish each send a single `interaction_required` and nothing else on its port. |
