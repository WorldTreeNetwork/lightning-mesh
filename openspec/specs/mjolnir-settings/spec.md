...
WAN-LAN SSH window timeout live in `/etc/config/mjolnir`. `mjolnir-apply`
is the only projector of those knobs onto OpenWrt UCI `wireless`. Folded
from `add-mjolnir-settings-store` (2026-09-10).

Code: `deploy/openwrt/files/etc/config/mjolnir`,
`deploy/openwrt/files/usr/sbin/mjolnir-apply`,
`deploy/openwrt/files/usr/sbin/mjolnir-wan-admin`,
`deploy/openwrt/setup-wireless.sh`.
